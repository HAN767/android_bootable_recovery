...
name := TWRP$(shell grep '#define TW_MAIN_VERSION_STR' $(LOCAL_PATH)/variables.h | sed -r 's#.*"(.*)".*#\1#')
ifeq ($(TARGET_BUILD_TYPE),debug)
  name := $(name)_debug
endif
name := $(name)-$(LINEAGE_VERSION)
RECOVERY_OTA_ZIP := $(PRODUCT_OUT)/$(name).zip
RECOVERY_OTA_FROM_TARGET_SCRIPT := $(PRODUCT_OUT)/recovery_ota_from_target_files.py
RECOVERY_OTA_FROM_TARGET_PATCH := $(LOCAL_PATH)/scripts/ota_from_target_files.patch

$(RECOVERY_OTA_FROM_TARGET_SCRIPT): $(OTA_FROM_TARGET_SCRIPT) $(RECOVERY_OTA_FROM_TARGET_PATCH)
	cp -a "$(OTA_FROM_TARGET_SCRIPT).py" "$@".tmp
